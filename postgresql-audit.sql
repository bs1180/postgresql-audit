...
CREATE OR REPLACE FUNCTION audit.jsonb_omit(val JSONB, ignore text[] = ARRAY[]::text[])
RETURNS JSONB AS $$
DECLARE
  result JSONB;
  v RECORD;
BEGIN
   ...
   FOR v IN SELECT * FROM jsonb_each_text(val) LOOP
     IF v.key = ANY(ignore)
...
CREATE OR REPLACE FUNCTION jsonb_diff(val1 JSONB, val2 JSONB, ignore text[] = ARRAY[]::text[])
RETURNS JSONB AS $$
DECLARE
  result JSONB;
  v RECORD;
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
        audit_row.row_data = jsonb_omit(to_jsonb(OLD), excluded_cols);
        audit_row.changed_fields = jsonb_diff(to_jsonb(NEW), to_jsonb(OLD), excluded_cols);
